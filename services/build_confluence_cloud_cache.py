...
def build_confluence_cloud_cache() -> dict:
    df = load_price_data()

    if df is None or df.empty:
        raise ValueError("Confluence Cloud cache build error: empty dataset")

    payload = build_confluence_cloud_latest_payload(df)

    if not payload:
        raise ValueError("Confluence Cloud cache build error: empty payload")
